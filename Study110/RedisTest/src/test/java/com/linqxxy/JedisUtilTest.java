...
//-----添加数据----------
        jedis.set("name","linqxxy");//向key-->name中放入了value-->linqxxy
        System.out.println(jedis.get("name"));//执行结果：linqxxy
//-----修改数据-----------
//1、在原来基础上修改
        jedis.append("name","jarorwar");   //很直观，类似map 将jarorwar append到已经有的value之后
        System.out.println(jedis.get("name"));//执行结果:linqxxyjarorwar
//2、直接覆盖原来的数据
        jedis.set("name","王文汉");
        System.out.println(jedis.get("name"));//执行结果：王文汉
//删除key对应的记录
        jedis.del("name");
        System.out.println(jedis.get("name"));//执行结果：null
/**
 * mset相当于
 * jedis.set("name","linqxxy");
 * jedis.set("jarorwar","王文汉");
 */
        jedis.mset("name","linqxxy","jarorwar","王文汉");
    ...
    public void testMap(){
        ...
        user.put("name","linqxxy");
        user.put("pwd","password");
        jedis.hmset("user",user);
//取出user中的name，执行结果:[linqxxy]-->注意结果是一个泛型的List
