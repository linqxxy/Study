...
import com.linqxxy.po.ItemsExample;
...
public class ItemsController {
    ...
    public String modifyInfoShow(Model model, HttpServletRequest request,
        ...
        if(id == null){
            ...
            items=itemsService.queryItemsById(Integer.valueOf(id));
        }
    ...
    public String editItems(Model model,HttpServletRequest request,
                                  HttpServletResponse response) throws Exception{
        String [] ids = request.getParameterValues("selItem");
        Integer [] idss = new Integer[ids.length];
        for (int i = 0 ; i < ids.length; i++) {
        ...
        List<ItemsCustom> itemsCustoms = itemsService.queryIdByIds(idss);
        model.addAttribute("itemsCustomsKey",itemsCustoms);
        return "editItems";
    }
    @RequestMapping(value = "/editSubmits")
    public String modifyInfoSubmits(ItemsVO itemsVO,HttpServletRequest request,HttpServletResponse response) throws Exception{
        System.out.println(itemsVO.getItemsList());
        return "success";
    }
}
